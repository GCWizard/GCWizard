...
void main() {
  group("triangle.triangleSidesMidPointsXY:", () {
    List<Map<String, Object?>> _inputsToExpected = [
      ...
      {'inputA': XYPoint(x: 1, y: 1), 'inputB': XYPoint(x: 2, y: 2), 'inputC': XYPoint(x: 3, y: 3),
        'expectedOutput': <XYPoint>[XYPoint(x: 2.5, y: 2.5), XYPoint(x: 2, y: 2), XYPoint(x: 1.5, y: 1.5)]},
    ...
    for (var elem in _inputsToExpected) {
      test('input: ${toTriangleObjectString(elem['inputA'])} ${toTriangleObjectString(elem['inputB'])} ${toTriangleObjectString(elem['inputC'])}', () {
        var _actual = triangleSidesMidPointsXY(elem['inputA'] as XYPoint, elem['inputB'] as XYPoint, elem['inputC'] as XYPoint);
