...
void main() {
  group("triangle.triangleCircumCircleXY:", () {
    List<Map<String, Object?>> _inputsToExpected = [
      ...
      {'inputA': XYPoint(x: 1, y: 1), 'inputB': XYPoint(x: 2, y: 2), 'inputC': XYPoint(x: 3, y: 3),
        'expectedOutput': XYCircle(x: 2, y: 2, r: 47453132.81212578)},
      {'inputA': XYPoint(x: 0, y: 0), 'inputB': XYPoint(x: 0, y: 3), 'inputC': XYPoint(x: 4, y: 0),
        'expectedOutput': XYCircle(x: 2, y: 1.4999999999999998, r: 2.5)},
    ];
    ...
    for (var elem in _inputsToExpected) {
      test('input: ${toTriangleObjectString(elem['inputA'])} ${toTriangleObjectString(elem['inputB'])} ${toTriangleObjectString(elem['inputC'])}', () {
        var _actual = triangleCircumscribedCircleXY(elem['inputA'] as XYPoint, elem['inputB'] as XYPoint, elem['inputC'] as XYPoint);
