...
void main() {
  group("triangle.triangleSymmediandPointsXY:", () {
    List<Map<String, Object?>> _inputsToExpected = [
      ...
      {'inputA': XYPoint(x: 0, y: 0), 'inputB': XYPoint(x: 0, y: 3), 'inputC': XYPoint(x: 4, y: 0),
        'expectedOutput': <XYPoint>[XYPoint(x: 2, y: 1.5), XYPoint(x: 2, y: 0), XYPoint(x: 0, y: 1.5)]},
    ];
    ...
    for (var elem in _inputsToExpected) {
      test('input: ${toTriangleObjectString(elem['inputA'])} ${toTriangleObjectString(elem['inputB'])} ${toTriangleObjectString(elem['inputC'])}', () {
        var _actual = triangleSymmediandPointsXY(elem['inputA'] as XYPoint, elem['inputB'] as XYPoint, elem['inputC'] as XYPoint);
