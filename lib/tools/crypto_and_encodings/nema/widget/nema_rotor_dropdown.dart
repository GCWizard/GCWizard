part of 'package:gc_wizard/tools/crypto_and_encodings/nema/widget/nema.dart';

class NEMARotorDropDown extends StatefulWidget {
  final void Function(Tuple2<int, EnigmaRotorConfiguration>) onChanged;
  final EnigmaRotorType type;
  final int position;

  const NEMARotorDropDown(
      {Key? key, required this.position, this.type = EnigmaRotorType.STANDARD, required this.onChanged})
      : super(key: key);

  @override
  _NEMARotorDropDownState createState() => _NEMARotorDropDownState();
}

class _NEMARotorDropDownState extends State<NEMARotorDropDown> {
