...
import 'package:gc_wizard/tools/science_and_technology/colors/xkcd_color_codes/logic/xkcd_color_codes.dart';

class XKCDColorCodes extends StatefulWidget {
  const XKCDColorCodes({super.key});

  @override
  _XKCDColorCodesState createState() => _XKCDColorCodesState();
}

class _XKCDColorCodesState extends State<XKCDColorCodes> {
  late MapEntry<String, XKCDColor> _currentValue;
  List<MapEntry<String, XKCDColor>> _colors = [];
  ...
  void initState() {
    ...
    _colors = XKCD_COLOR_CODES.entries.toList();
  ...
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[
        GCWTwoOptionsSwitch(
          value: _currentMode,
          leftValue: i18n(context, 'xkcdcolorcodes_mode_raltocolor'),
          rightValue: i18n(context, 'xkcdcolorcodes_mode_colortoral'),
          onChanged: (value) {
            setState(() {
              _currentMode = value;
            });
        ...
        _currentMode == GCWSwitchPosition.left
            ? Column(
          children: [
            GCWDropDown<MapEntry<String, XKCDColor>>(
                value: _currentValue,
                onChanged: (newValue) {
                  setState(() {
  ...
  List<Object> _buildRALColorOutput(MapEntry<String, XKCDColor> ral) {
  ...
  Widget _buildColorToRALOutput() {
    ...
    if (similarRALs.isEmpty) {
      return GCWDefaultOutput(
        child: i18n(context, 'xkcdcolorcodes_mode_colorstoral_nocolorfound'),
      );
    }
    ...
    return GCWOutput(
      title: similarRALs.length == 1
          ...
          : i18n(context, 'xkcdcolorcodes_mode_colorstoral_similarcolorsfound'),
  ...
  Widget _buildRALToColorOutput() {
    ...
    children.add(GCWButton(
      text: i18n(context, 'xkcdcolorcodes_showincolorpicker'),
