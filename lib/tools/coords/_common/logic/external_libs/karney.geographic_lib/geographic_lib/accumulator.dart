...
// taken from official Java port https://github.com/geographiclib/geographiclib-java/blob/main/src/main/java/net/sf/geographiclib/Accumulator.java
...
class _Accumulator {
  ...
  double _s = 0.0, _t = 0.0;
  /**
   * Construct from a double.
   * <p>
   * @param y set <i>sum</i> = <i>y</i>.
   **********************************************************************/
  _Accumulator(double y) { _s = y; _t = 0; }
  /**
   * Construct from another Accumulator.
   * <p>
   * @param a set <i>sum</i> = <i>a</i>.
   **********************************************************************/
  static _Accumulator fromAccumulator(_Accumulator a) {
    var newAccumulator = _Accumulator(a._s);
    newAccumulator._t = a._t;
   ...
   **********************************************************************/
  double _Sum({double y = double.nan}) {
    if (y.isNaN) {
      return _s;
    }
    _Pair p = _Pair();
    _AddInternal(p, _s, _t, y);
    return p.first;
  }
  /**
   ...
   **********************************************************************/
  static void _AddInternal(_Pair p, double s, double t, double y) {
    // Here's Shewchuk's solution...
    double u;                       // hold exact sum as [s, t, u]
    // Accumulate starting at least significant end
    _sum(p, y, t); y = p.first; u = p.second;
    _sum(p, y, s); s = p.first; t = p.second;
   ...
   **********************************************************************/
  void _Remainder(double y) {
    _s = ieeeRemainder(_s, y).toDouble();
    _Add(0.0);                   // renormalize
  }
}